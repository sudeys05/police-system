...
// Reports Collection CRUD
export const ReportsCRUD = {
  async create(reportData) {
    console.log('🔍 Creating report in MongoDB with data:', reportData);
    const db = getDatabase();
    
    // Generate report number if not provided
    const reportNumber = reportData.reportNumber || `RPT-${new Date().getFullYear()}-${Math.random().toString(36).substr(2, 8).toUpperCase()}`;
    ...
    console.log('📝 Report document to insert:', docToInsert);
    
    const result = await db.collection('reports').insertOne(docToInsert);
    console.log('✅ Report insert result:', { insertedId: result.insertedId, acknowledged: result.acknowledged });
    
    // Return the actual document from database
    const insertedDoc = await db.collection('reports').findOne({ _id: result.insertedId });
    console.log('📄 Retrieved inserted report document:', insertedDoc);
  ...
  async findByReportNumber(reportNumber) {
    console.log('🔍 Finding report by report number:', reportNumber);
    const db = getDatabase();
    return await db.collection('reports').findOne({ reportNumber });
  },

  async findByCaseId(caseId) {
    console.log('🔍 Finding reports by case ID:', caseId);
    const db = getDatabase();
    return await db.collection('reports').find({ caseId }).toArray();
  },

  async findByOBId(obId) {
    console.log('🔍 Finding reports by OB ID:', obId);
    const db = getDatabase();
    return await db.collection('reports').find({ obId }).toArray();
  },
  ...
  async findByStatus(status) {
    console.log('🔍 Finding reports by status:', status);
    const db = getDatabase();
    return await db.collection('reports').find({ status }).toArray();
  },

  async findByType(type) {
    console.log('🔍 Finding reports by type:', type);
    const db = getDatabase();
    return await db.collection('reports').find({ type }).toArray();
  },

  async findByRequestedBy(requestedBy) {
    console.log('🔍 Finding reports by requested by:', requestedBy);
    const db = getDatabase();
    return await db.collection('reports').find({ requestedBy }).toArray();
  },
  ...
  async update(id, updateData) {
    ...
    // Remove fields that shouldn't be directly updated
    const { reportNumber, createdAt, ...safeUpdateData } = updateData;
    
    const result = await db.collection('reports').updateOne(
      { _id: new ObjectId(id) },
      { $set: { ...safeUpdateData, updatedAt: new Date() } }
    );
    console.log('📝 Report update result:', { modifiedCount: result.modifiedCount });
    return result.modifiedCount > 0;
  },
  ...
  async delete(id) {
    ...
    console.log('📝 Report delete result:', { deletedCount: result.deletedCount });
    return result.deletedCount > 0;
  },
  ...
  async getReportStats() {
    ...
    const db = getDatabase();
    const stats = await db.collection('reports').aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: 1 },
